"""
Qwen1.5-7B (未微调) — BiomniEval1 基线评测（本地模型）
用途：使用已经下载好的 Qwen/Qwen1.5-7B-Chat 权重或本地缓存的权重，逐条运行 BiomniEval1 验证集并保存结果。
用法示例：
  # 使用 HF 名称从缓存/远端加载（需要网络/token）
  LOCAL_MODEL_DIR="Qwen/Qwen1.5-7B-Chat" python scripts/run_qwen_base_baseline.py
  # 或指定本地已经下载好的模型目录
  LOCAL_MODEL_DIR="~/.cache/huggingface/hub/models--Qwen--Qwen1.5-7B-Chat" python scripts/run_qwen_base_baseline.py

环境变量（可选）：
  LOCAL_MODEL_DIR     本地模型路径或 HF 名称（默认 "Qwen/Qwen1.5-7B-Chat"）
  OUTPUT_DIR          结果输出目录（默认 "../results/qwen_base"）
  LOCAL_PARQUET       评测数据路径（默认 "../data/biomni_eval1_dataset.parquet"）
  LOAD_IN_8BIT        若为 "1"/"true"，尝试使用 bitsandbytes 8-bit 加载（默认 false）
  SUBSET              若为正整数，仅对每个任务运行前 N 个样例（调试用）
  GEN_MAX_NEW_TOKENS  生成最大 token（默认 512）
  GEN_TEMPERATURE     temperature（默认 0.0）
  SLEEP_BETWEEN       每次调用后的 sleep（��，默认 0.1）
"""
import os
import re
import time
...
# --------------- 配置（可由环境变量覆盖） ---------------
LOCAL_MODEL_DIR = os.getenv("LOCAL_MODEL_DIR", "Qwen/Qwen1.5-7B-Chat")
OUTPUT_DIR = os.getenv("OUTPUT_DIR", "../results/qwen_base")
LOCAL_PARQUET = os.getenv("LOCAL_PARQUET", "../data/biomni_eval1_dataset.parquet")
LOAD_IN_8BIT = os.getenv("LOAD_IN_8BIT", "false").lower() in ("1", "true", "yes")
SUBSET = int(os.getenv("SUBSET", "0"))  # 0 表示不截断
...
# --------------- 答案提取与评分（复用原 run_deepseek_baseline.py 逻辑） ---------------
...
try:
    ...
    print("Loaded model with device_map='auto' and torch_dtype=float16.")
except Exception as e:
    load_errors.append(("float16_auto", str(e)))
    print("float16/device_map load failed:", e)

# 若失败并且允��� 8-bit 尝试
if model is None and LOAD_IN_8BIT:
    try:
        model = AutoModelForCausalLM.from_pretrained(
            LOCAL_MODEL_DIR,
            trust_remote_code=True,
        ...
        print("Loaded model in 8-bit mode (bitsandbytes).")
    except Exception as e:
        load_errors.append(("8bit_auto", str(e)))
        print("8-bit load failed:", e)

# 最后使用 low_cpu_mem_usage fallback
if model is None:
    try:
        model = AutoModelForCausalLM.from_pretrained(
            LOCAL_MODEL_DIR,
        ...
        if torch.cuda.is_available():
            model.to("cuda")
        print("Loaded model with low_cpu_mem_usage fallback.")
    except Exception as e:
        load_errors.append(("low_cpu_mem_usage", str(e)))
        print("Fallback load failed:", e)
        print("All load attempts failed. Errors:", load_errors)
        raise
...
def main():
    print(f"{'='*60}")
    print("Qwen1.5-7B Base Model Local Baseline Evaluation")
    ...
    print("\n\n" + "=" * 60)
    print("FINAL RESULTS — Qwen Base Local")
    print("=" * 60)
