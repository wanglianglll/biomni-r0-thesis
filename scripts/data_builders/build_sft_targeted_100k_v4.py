...
OUT = ROOT / "data/sft_targeted_100k_v4"
...
SPECS = [
    ...
    ("D5", ROOT / "data/sft_d5_gwas_variant_v3/d5_gwas_variant_train.jsonl", 20000),
    ("D6", ROOT / "data/sft_d6_gwas_causal_gene_v2/d6_gwas_causal_gene_train.jsonl", 15000),
]
...
def normalize_schema(obj: dict, mixture_part: str) -> dict:
    return {
        ...
        "mixture_dataset": "sft_targeted_100k_v4",
...
def main() -> None:
    ...
    all_rows = []
    summary = {
        "dataset": "sft_targeted_100k_v4",
        "seed": args.seed,
        "policy": "D1-heavy mixture with D4 v3, D5 v3, D6 v2. Assistant messages are single FINAL ANSWER lines.",
        "parts": {},
    }
    global_seen = set()
    ...
    (args.out / "README.md").write_text(
        "# SFT Targeted 100K V4\n\n"
        "A short-validation SFT mixture using repaired D4/D5/D6 data.\n\n"
        "- D1: 45K direct datalake samples\n"
        "- D2: 4K contrast samples\n"
        "- D3: 4K verifiable samples\n"
        "- D4: 10K rare-disease targeted v3 samples\n"
        "- D5: 20K GWAS variant-prioritization targeted v3 samples\n"
        "- D6: 15K GWAS causal-gene targeted v2 samples\n",
        encoding="utf-8",
    )
