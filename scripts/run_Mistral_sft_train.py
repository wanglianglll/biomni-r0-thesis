"""
Mistral-7B-Instruct 全参数SFT微调训练脚本
所有路径均写死，无需命令行参数。
"""
...
# 路径配置（本地模型为未微调的 Mistral-7B-Instruct，需确保目录权重完整）
MODEL_DIR = "/root/autodl-tmp/Biomni-main/models/Mistral-7B-Instruct-v0.3"
TRAIN_FILE = "/root/autodl-tmp/Biomni-main/data/sft/sft_train.jsonl"
OUTPUT_DIR = "/root/autodl-tmp/Biomni-main/scripts/output/mistral_sft"
...
# 加载/预处理数据集（jsonl格式，每条含"messages"，ChatML结构）
...
def preprocess(example):
    ...
    result["labels"] = result["input_ids"]  # 必须加labels供SFT训练
    return result
