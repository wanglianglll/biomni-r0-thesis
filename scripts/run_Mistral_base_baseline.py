"""
Mistral-7B (未微调) — BiomniEval1 基线评测
用途：使用未微调的 mistralai/Mistral-7B-v0.1 权重，逐条评测并保存结果。
结果输出目录：results/Mistral
"""
import os
import re
import time
...
# 配置
LOCAL_MODEL_DIR = "/root/autodl-tmp/Biomni-main/models/Mistral-7B"
# LOCAL_MODEL_DIR = os.getenv("LOCAL_MODEL_DIR", "mistralai/Mistral-7B-v0.1")
OUTPUT_DIR = os.getenv("OUTPUT_DIR", "results/Mistral")
...
print("Loading Mistral-7B tokenizer and model (this may take a while)...")
...
def main():
    print("=" * 60)
    print("Mistral-7B Base Model Local Baseline Evaluation")
